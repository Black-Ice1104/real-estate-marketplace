...
export default function LikeUnlike({ ad }) {
  ...
  const handleLike = async (adId) => {
    if (auth.user === null) {
        navigate("/login", { // if click like before login, will navigate to login
          state: `/ad/${ad.slug}`, // after login, redirect to the original page (ad or dashboard)
        });
        return;
    }
    try {
